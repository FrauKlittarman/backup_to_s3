...
if __name__ == "__main__":
    try:
        ...
        if arg and arg == "job-end":
            logger.info(f'str dadatime hour is "{str(datetime.datetime.now().hour)}"')
            if str(datetime.datetime.now().hour) in [0, 1, 2]:
                ...
                for FileName in BACKUP_FILENAME_LIST:
                    if str(curent_date) in FileName and not FileName.endswith(".log"):
                ...
                logger.info("Currently, it is not a good time for cloud backups.")
